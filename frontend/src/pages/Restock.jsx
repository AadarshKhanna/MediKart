...
function Restock() {
  ...
  const handleRestock = async (medicineId) => {
    ...
    try {
      await axios.patch(`http://localhost:5001/api/medicines/restock/${medicineId}`, {
  ...
  return (
    <div className="min-h-screen w-full p-8 bg-gray-50 dark:bg-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/30 rounded-lg flex items-center gap-2 text-red-300 dark:text-red-400">
            <AlertCircle size={20} />
            <span>{error}</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMedicines.map((medicine) => (
            <motion.div
              ...
              className="bg-white dark:bg-amber-800 rounded-xl shadow-lg overflow-hidden"
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={`px-3 py-1.5 rounded-lg text-sm font-medium ${
                    medicine.quantity <= medicine.restockThreshold
                      ? "bg-red-100 text-red-700 dark:bg-red-000/30 dark:text-red-400"
